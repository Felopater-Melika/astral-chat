...
describe('MessageController_class', () => {
  ...
  it("test_create_returns_message_object",async () => {
    // Arrange
    const createMessageDto = { senderId: '1', conversationId: '1', body: 'test' };
    const user = { id: '1' };
    const message = { id: '1', body: 'test', senderId: '1', conversationId: '1' };
    ...
    expect(messageService.create).toHaveBeenCalledWith(createMessageDto, user.id);
  })
  ...
  it("test_find_all_returns_array_of_message_objects",async () => {
    // Arrange
    const user = { id: '1' };
    const messages = [{ id: '1', body: 'test', senderId: '1', conversationId: '1' }];
    // eslint-disable-next-line @typescript-eslint/ban-ts-comment
    // @ts-ignore
    const messageService = new MessageService();
    const messageController = new MessageController(messageService);
    ...
    // Act
    const result = await messageController.findAll(user);

    // Assert
    expect(result).toBe(messages);
    expect(messageService.findAll).toHaveBeenCalledWith(user.id);
  ...
  it("test_create_throws_NotFoundException_if_conversation_not_found",async () => {
    ...
    const messageService = new MessageService();
    const messageController = new MessageController(messageService);
    jest.spyOn(messageService, 'create').mockRejectedValueOnce(new NotFoundException());

    // Act and Assert
    await expect(messageController.create(createMessageDto, user)).rejects.toThrow(NotFoundException);
